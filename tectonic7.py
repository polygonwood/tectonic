				...
				for n in ne[:]:
					...
					for on in ne[:]:
						if on <> n: # it's a different neighbour
							...
							if onB == nB and on.notQueued(): # from same block and without finale value yet
								...
								#print ny,nx,on.getY(),on.getX(),commonV
					...
					for be in eB[:]:
						if be not in ne and be.notQueued(): # it's not a neighbour and has not yet final value set
							...
							#print ny,nx,be.getY(),be.getX(),commonV
					...
					for p in commonV[:]:
						...
						print 'before'
						for t in self.rosterBlock[x-1][y-1].targets[:]:
							print n.getY(),n.getX(),
							for e in t[:]:
								print e.getY(),e.getX(),'-',
							print ' '
...
t.append(Block(3,[1, 1], [2, 1], [1, 2]))
t.append(Block(5,[3, 1], [4, 1], [2,2], [3, 2], [4, 2]))
t.append(Block(5,[5, 1], [6, 1], [5, 2], [6, 2], [6, 3]))
t.append(Block(5,[1, 3], [2, 3],[1,4],[2,4],[3,4]))
t.append(Block(5,[3, 3], [4, 3], [5, 3], [4, 4], [5, 4]))
t.append(Block(4,[1, 5], [2, 5], [3, 5], [4, 5]))
t.append(Block(5,[6,4], [5, 5], [6, 5], [5, 6],[6,6]))
t.append(Block(5,[1, 6], [2, 6], [1, 7], [2, 7], [3, 7]))
t.append(Block(5,[3, 6], [4, 6], [4, 7], [5, 7], [6, 7]))
t.printall()
if t.check():
	exit()
t.setNeighbours()
...
valueq = [[6,1,5],[1,2,3],[5,2,3],[2,3,5],[6,3,4],[4,4,1],[6,6,3],[2,7,1],[4,7,1],[6,7,2]]
t.solve(valueq)
t.printRoster()
